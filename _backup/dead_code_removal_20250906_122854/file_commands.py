...
class ChooseFilesCommand(BaseCommand):
    ...
    def _execute_impl(self) -> CommandResult:
        ...
        try:
            ...
            if not files:
                return CommandResult(success=False, message="파일이 선택되지 않았습니다.")
            ...
            return CommandResult(
                success=True, message=f"{len(files)}개 파일이 선택되었습니다.", data=files
            )

        except Exception as e:
            return CommandResult(success=False, message=f"파일 선택 실패: {str(e)}", error=e)
    ...
    def _execute_impl(self) -> CommandResult:
        ...
        try:
            ...
            folder = QFileDialog.getExistingDirectory(
                self.parent_widget, "애니메이션 폴더 선택", ""
            )

            if not folder:
                return CommandResult(success=False, message="폴더가 선택되지 않았습니다.")
            ...
            return CommandResult(
                success=True, message=f"폴더가 선택되었습니다: {folder}", data=folder
            )

        except Exception as e:
            return CommandResult(success=False, message=f"폴더 선택 실패: {str(e)}", error=e)
        ...
        try:
            ...
            if self.event_bus:
                if self.file_paths:
                    ...
                    return CommandResult(
                        success=False, message="스캔할 파일이나 폴더가 지정되지 않았습니다."
                    )

            return CommandResult(success=True, message="파일 스캔을 시작합니다.")

        except Exception as e:
            return CommandResult(success=False, message=f"스캔 시작 실패: {str(e)}", error=e)
...
class StopScanCommand(BaseCommand):
    ...
    def _execute_impl(self) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(success=True, message="파일 스캔을 중지합니다.")

        except Exception as e:
            return CommandResult(success=False, message=f"스캔 중지 실패: {str(e)}", error=e)
