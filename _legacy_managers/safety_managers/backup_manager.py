...
# from src.core.events import (BackupCleanupEvent, BackupCompletedEvent,
...
class BackupManager:
    ...
    def create_backup(
        ...
        self.logger.info(f"백업 시작: {backup_id} -> {backup_location}")
        ...
        try:
            ...
            # 새로운 이벤트 시스템으로 변경 - 로그만 남김
            self.logger.error(f"백업 실패: {backup_id} - {e}")
    ...
    def cleanup_old_backups(self, max_age_days: int = None, max_backups: int = None) -> int:
        ...
        if cleaned_count > 0:
            # 새로운 이벤트 시스템으로 변경 - 로그만 남김
            self.logger.info(f"백업 정리 완료: {cleaned_count}개, {freed_space} bytes 해제")
            # 필요시 event_publisher.publish_organize_completed() 등을 사용
