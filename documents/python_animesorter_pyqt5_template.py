...
from PyQt5.QtCore import Qt, QAbstractTableModel, QModelIndex, QVariant, QSortFilterProxyModel, QTimer, pyqtSignal
...
class ItemsTableModel(QAbstractTableModel):
    headers = [
        "Status", "Title / Path", "S", "E", "Year", "Resolution", "Size", "TMDB", "Actions"
    ]
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        line = QWidget(); hl = QHBoxLayout(line); hl.setContentsMargins(0,0,0,0)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.search = QLineEdit(); self.search.setPlaceholderText("Search title, path…")
        self.statusFilter = QComboBox(); self.statusFilter.addItems(["all", "parsed", "needs_review", "error"])
        ...
        self.progressBar = QProgressBar(); self.progressBar.setValue(0)
        ...
        row_l.addWidget(self.btnStart); row_l.addWidget(self.btnPause)
        qa_l.addWidget(row)
        ...
        self.cmbResolution = QComboBox(); self.cmbResolution.addItems(["All", "2160p", "1080p", "720p"]) # stub only
        self.cmbContainer = QComboBox(); self.cmbContainer.addItems(["All", "MKV", "MP4"]) # stub only
        self.cmbCodec = QComboBox(); self.cmbCodec.addItems(["All", "HEVC", "H.264"]) # stub only
        self.cmbYear = QComboBox(); self.cmbYear.addItems(["All", "2025", "2024", "2023"]) # stub only
        ...
        rh = QWidget(); rh_l = QHBoxLayout(rh); rh_l.setContentsMargins(0,0,0,0)
        ...
        self.tree.setHeaderLabels(["Title / Season / Episode", "Files", "Best Res"])  # minimal columns
        ...
        # Footer actions
        rf = QWidget(); rf_l = QHBoxLayout(rf); rf_l.setContentsMargins(0,0,0,0)
        self.lblShowing = QLabel("")
        rf_l.addWidget(self.lblShowing)
        rf_l.addStretch(1)
        self.btnCommit = QPushButton("Commit organize")
        self.btnCommit.clicked.connect(lambda: QMessageBox.information(self, "Commit", "Commit organization plan (stub)"))
        self.btnSimulate = QPushButton("Simulate")
        self.btnSimulate.clicked.connect(lambda: QMessageBox.information(self, "Simulate", "Simulate file moves (stub)"))
        ...
        self.txtErr = QTextEdit(); self.txtErr.setReadOnly(True)
        self.txtLog.setText("""[12:00:01] Watching /downloads ...\n[12:00:04] Parsed Frieren S01E01 (score=0.92, tmdb=209867)\n[12:00:08] Found potential special episode; awaiting review\n[12:00:17] Ready to organize 1 item (copy mode)""")
        self.txtErr.setText("""[12:01:12] Failed TMDB match for Unknown.Show.12v2.mp4 — try manual search with an alias.""")
        ...
        mm_l = QHBoxLayout(mm)
        self.leManual = QLineEdit(); self.leManual.setPlaceholderText("Search TMDB for title (ja/en/ko)")
        btnMMSearch = QPushButton("Search")
        btnMMAttach = QPushButton("Attach")
        btnMMSearch.clicked.connect(lambda: QMessageBox.information(self, "TMDB", "Search TMDB (stub)"))
        btnMMAttach.clicked.connect(lambda: QMessageBox.information(self, "TMDB", "Attach metadata (stub)"))
        ...
        # Root layout
        root = QWidget(); root_l = QVBoxLayout(root)
        root_l.addWidget(top)
        line = QFrame(); line.setFrameShape(QFrame.HLine); root_l.addWidget(line)
    ...
    def clearCompleted(self):
        ...
        self.refreshStats(); self.refreshShowing(); self.rebuildGroupedView()
    ...
    def resetFilters(self):
        ...
        self.search.clear(); self.statusFilter.setCurrentText("all")

    def bulkActions(self):
        QMessageBox.information(self, "Bulk actions", "- TMDB auto-match\n- Organize selected\n- Export CSV\n(Stub)")
    ...
    def rebuildGroupedView(self):
        ...
        for (title, season, episode), files in sorted(groups.items(), key=lambda k: (k[0][0].lower(), k[0][1], k[0][2])):
            ...
            if s_key not in season_nodes:
                season_item = QTreeWidgetItem([f"{title} – Season {season}", "", ""])  # title at season level
