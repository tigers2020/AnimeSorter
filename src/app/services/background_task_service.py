...
class BackgroundTaskService(QObject):
    ...
    def __init__(
        ...
        # 작업 관리
        self.tasks: dict[str, BaseTask] = {}  # task_id -> BaseTask
        self.task_statuses: dict[str, TaskStatus] = {}  # task_id -> TaskStatus
        self.pending_queue: deque[BaseTask] = deque()  # 우선순위 대기열
        ...
        # 상태 업데이트 타이머 (필요시에만 활성화)
        self.status_timer = QTimer()
        self.status_timer.timeout.connect(self._publish_queue_status)
        # self.status_timer.start(5000)  # 스패밍 방지를 위해 비활성화
