...
class SearchResultMatcher:
    ...
    def _auto_match(
        ...
        try:
            ...
            if scored_results and scored_results[0][1] > 0.6:
                ...
                return TMDBMatch(
                    search_result=search_result, confidence=confidence, confidence_score=best_score
                )
    ...
    def _manual_match(
        ...
        if search_results:
            ...
            return TMDBMatch(
                search_result=search_result, confidence=confidence, confidence_score=score
            )
        return None
    ...
    def _calculate_title_similarity(self, group_title: str, tmdb_title: str) -> float:
        ...
        if group_lower in tmdb_lower or tmdb_lower in group_lower:
            return 0.9
        group_words = set(re.findall("\\w+", group_lower))
        tmdb_words = set(re.findall("\\w+", tmdb_lower))
    ...
    def _calculate_genre_score(self, genres: list[dict[str, Any]]) -> float:
        ...
        if not genres:
            return 0.0
        anime_genre_ids = [16, 10759, 10762, 10765]
        for genre in genres:
