...
class FileManager:
    ...
    def _create_backup(self, source_path: Path, operation_id: str) -> Path | None:
        """파일 백업 생성"""
        ...
        try:
            ...
            if operation == "copy":
                ...
                if result.success and backup_path and operation_id in self.backup_metadata:
                    self.backup_metadata[operation_id]["final_destination_path"] = str(
                        destination_path
                    )
    ...
    def _generate_destination_path(self, source_file: Path, metadata: dict) -> Path | None:
        """대상 파일 경로 생성"""
        try:
            # 기본 정보 추출
    ...
    def rename_file(self, old_path: str, new_name: str) -> FileOperationResult:
        ...
        try:
            ...
            return FileOperationResult(
                success=False, source_path=old_path, error_message=str(e), operation_type="rename"
            )
    ...
    def delete_file(self, file_path: str) -> FileOperationResult:
        ...
        try:
            ...
            return FileOperationResult(
                success=True, source_path=file_path, operation_type=operation_type
            )

        except Exception as e:
            self.logger.error(f"파일 삭제 실패: {e}")
            return FileOperationResult(
                success=False, source_path=file_path, error_message=str(e), operation_type="delete"
            )
