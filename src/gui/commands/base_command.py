...
class BaseCommand(ICommand, ABC):
    ...
    def execute(self) -> CommandResult:
        ...
        try:
            ...
            # 이벤트 버스에 실행 완료 알림
            if self.event_bus:
                self.event_bus.publish("command_executed", {
            ...
            # 이벤트 버스에 오류 알림
            if self.event_bus:
                self.event_bus.publish("command_failed", {
                    "command_name": self.name,
                    "error": str(e),
    ...
    def undo(self) -> CommandResult:
        ...
        try:
            ...
            # 이벤트 버스에 실행 취소 알림
            if self.event_bus:
                self.event_bus.publish("command_undone", {
                    "command_name": self.name
                })
