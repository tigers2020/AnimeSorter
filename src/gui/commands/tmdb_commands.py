    ...
    def _execute_impl(self) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                success=True,
                message="TMDB 검색을 시작합니다."
            )
            
        except Exception as e:
            return CommandResult(
                success=False,
                message=f"TMDB 검색 시작 실패: {str(e)}",
                error=e
            )
...
class SelectTMDBAnimeCommand(BaseCommand):
    ...
    def _execute_impl(self) -> CommandResult:
        ...
        try:
            ...
            if self.event_bus:
                self.event_bus.publish("tmdb_anime_selected", {
            ...
            return CommandResult(
                success=False,
                message=f"TMDB 애니메이션 선택 실패: {str(e)}",
                error=e
            )
    ...
    def _save_state_for_undo(self):
        ...
        if self.event_bus:
            ...
            self._undo_data = {
                "group_key": self.group_key,
                "previous_match": self._previous_match
            }
        else:
            self._can_undo = False
            self._undo_data = None
    ...
    def _undo_impl(self) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                success=True,
                message=f"TMDB 애니메이션 선택 취소됨: {self.group_key}"
            )
            
        except Exception as e:
            return CommandResult(
                success=False,
                message=f"TMDB 애니메이션 선택 취소 실패: {str(e)}",
                error=e
            )
...
class SkipTMDBGroupCommand(BaseCommand):
    ...
    def _execute_impl(self) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                success=True,
                message=f"TMDB 그룹 건너뛰기: {self.group_key}"
            )
            
        except Exception as e:
            return CommandResult(
                success=False,
                message=f"TMDB 그룹 건너뛰기 실패: {str(e)}",
                error=e
            )
