...
class MainWindowInitializer:
    ...
    def _init_core_components(self):
        """핵심 컴포넌트 초기화"""
        try:
            # 설정 관리자 설정 (통합 설정 시스템 사용)
            self.main_window.settings_manager = unified_config_manager
            ...
            # FileManager 초기화
            dest_root = unified_config_manager.get_setting("destination_root", "")
            safe_mode = unified_config_manager.get_setting("safe_mode", True)
            ...
            # FileManager 설정 적용
            naming_scheme = unified_config_manager.get_setting("naming_scheme", "standard")
    ...
    def apply_settings_to_ui(self):
        ...
        try:
            # 기본 설정 적용
            self.main_window.organize_mode = unified_config_manager.get_setting(
                "organize_mode", "이동"
            )
            self.main_window.naming_scheme = unified_config_manager.get_setting(
                "naming_scheme", "standard"
            )
            self.main_window.safe_mode = unified_config_manager.get_setting("safe_mode", True)
            self.main_window.backup_before_organize = unified_config_manager.get_setting(
                "backup_before_organize", False
            )
            self.main_window.prefer_anitopy = unified_config_manager.get_setting(
                "prefer_anitopy", True
            )
            self.main_window.fallback_parser = unified_config_manager.get_setting(
                "fallback_parser", "GuessIt"
            )
            self.main_window.realtime_monitoring = unified_config_manager.get_setting(
                "realtime_monitoring", False
            )
            self.main_window.auto_refresh_interval = unified_config_manager.get_setting(
                "auto_refresh_interval", 30
            )
            self.main_window.tmdb_language = unified_config_manager.get_setting(
                "tmdb_language", "ko-KR"
            )
            self.main_window.show_advanced_options = unified_config_manager.get_setting(
                "show_advanced_options", False
            )
            self.main_window.log_level = unified_config_manager.get_setting("log_level", "INFO")
            self.main_window.log_to_file = unified_config_manager.get_setting("log_to_file", False)
            self.main_window.backup_location = unified_config_manager.get_setting(
                "backup_location", ""
            )
            self.main_window.max_backup_count = unified_config_manager.get_setting(
                "max_backup_count", 10
            )
