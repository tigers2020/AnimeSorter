...
TMDB 검색이 완료된 후 다시 시도해주세요.""",
            ...
            logger.info("❌ 정리 실행 요청 처리 실패: %s", e)
    ...
    def on_settings_opened(self):
        """설정 열기 처리"""
        try:
            logger.info("⚙️ 설정 열기 요청됨")
            self.show_settings_dialog()
        except Exception as e:
            logger.info("❌ 설정 열기 처리 실패: %s", e)
    ...
    def on_source_folder_selected(self, folder_path: str):
        ...
        try:
            ...
            logger.info("❌ 소스 폴더 선택 처리 실패: %s", e)
    ...
    def on_source_files_selected(self, file_paths: list[str]):
        ...
        try:
            ...
            logger.info("❌ 소스 파일 선택 처리 실패: %s", e)
    ...
    def on_destination_folder_selected(self, folder_path: str):
        ...
        try:
            ...
            logger.info("❌ 대상 폴더 선택 처리 실패: %s", e)
    ...
    def show_about(self):
        ...
        try:
            ...
            logger.info("❌ 정보 다이얼로그 표시 실패: %s", e)
    ...
    def show_help(self):
        ...
        try:
            ...
            logger.info("❌ 사용법 다이얼로그 표시 실패: %s", e)
    ...
    def show_settings_dialog(self):
        ...
        try:
            ...
            logger.info("❌ 설정 다이얼로그 표시 실패: %s", e)
            QMessageBox.critical(
        ...
        try:
            if hasattr(self.main_window, "accessibility_manager"):
                ...
                if high_contrast != self.main_window.accessibility_manager.high_contrast_mode:
                    ...
                    logger.info("✅ 고대비 모드: %s", "활성화" if high_contrast else "비활성화")
            ...
            logger.info("⚠️ 접근성 설정 적용 실패: %s", e)
        ...
        try:
            if hasattr(self.main_window, "i18n_manager"):
                ...
                if new_language != self.main_window.i18n_manager.get_current_language():
                    ...
                    logger.info("✅ 언어가 '%s'로 변경되었습니다.", new_language)
        except Exception as e:
            logger.info("⚠️ 언어 설정 적용 실패: %s", e)
    ...
    def _reinitialize_tmdb_client(self):
        ...
        try:
            ...
            logger.info("⚠️ TMDB 클라이언트 재초기화 실패: %s", e)
    ...
    def get_accessibility_info(self) -> dict:
        ...
        try:
            ...
            logger.info("❌ 접근성 정보 반환 실패: %s", e)
        return {}

    def on_language_changed(self, language_code: str):
        """언어 변경 이벤트 처리"""
        try:
            logger.info("🌍 언어가 %s로 변경되었습니다", language_code)
            ...
            logger.info("❌ 언어 변경 이벤트 처리 실패: %s", e)
    ...
    def _update_ui_texts(self):
        ...
        try:
            ...
            logger.info("⚠️ UI 텍스트 업데이트 실패: %s", e)
    ...
    def _update_toolbar_texts(self, tr):
        ...
        try:
            ...
            logger.info("⚠️ 툴바 텍스트 업데이트 실패: %s", e)
    ...
    def _update_tab_texts(self, tr):
        ...
        try:
            ...
            logger.info("⚠️ 탭 텍스트 업데이트 실패: %s", e)
    ...
    def get_supported_languages(self) -> dict:
        ...
        try:
            ...
            logger.info("❌ 지원 언어 목록 반환 실패: %s", e)
