...
class FileOrganizeWorker(QThread):
    ...
    def __init__(self, grouped_items: Dict[str, List], destination_directory: str):
        ...
        # 자막 파일 확장자 정의
        self.subtitle_extensions = {'.srt', '.ass', '.ssa', '.sub', '.vtt', '.idx', '.smi', '.sami', '.txt'}
    ...
    def run(self):
        ...
        try:
            ...
            for group_id, group_items in self.grouped_items.items():
                ...
                # 대상 디렉토리 경로 생성 및 검증
                target_base_dir = os.path.join(self.destination_directory, safe_title, season_folder)
                    ...
                    try:
                        ...
                        for subtitle_path in subtitle_files:
                            try:
                                ...
                                subtitle_target_path = self._resolve_target_path(target_base_dir, subtitle_filename)
                                ...
                                self.file_processed.emit(subtitle_filename, f"자막 이동 완료: {subtitle_target_path}", True)
                                
                            except Exception as e:
                                error_msg = f"자막 파일 이동 실패: {subtitle_path} - {str(e)}"
                                result.errors.append(error_msg)
                                self.file_processed.emit(os.path.basename(subtitle_path), error_msg, False)
    ...
    def _sanitize_title(self, representative):
        ...
        try:
            # 제목 추출
            if hasattr(representative, 'tmdbMatch') and representative.tmdbMatch and representative.tmdbMatch.name:
...
class OrganizeProgressDialog(QDialog):
    ...
    def on_organization_completed(self, result: OrganizeResult):
        ...
        if result.cleaned_directories > 0:
            self.log_text.append(f"🗑️ 빈 디렉토리: {result.cleaned_directories}개 디렉토리 정리 완료")
    ...
    def get_result(self) -> Optional[OrganizeResult]:
        """결과 반환"""
        return self.result
