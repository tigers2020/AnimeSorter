...
from PyQt5.QtWidgets import (
    ...
    QListWidgetItem, QLineEdit, QGroupBox, QTextEdit, QFrame, QScrollArea,
...
class TMDBSearchDialog(QDialog):
    ...
    def on_search_completed(self, results: List[TMDBAnimeInfo]):
        ...
        if len(results) == 1:
            ...
            self.lblSearchStatus.setText(f"검색결과 1개 - 자동 선택됨")
        ...
        for anime in results:
            try:
                ...
                # 간단한 텍스트 아이템으로 대체
                simple_item = QListWidgetItem(f"ID: {anime.id} - {getattr(anime, 'name', 'Unknown')}")
    ...
    def create_result_item_widget(self, anime: TMDBAnimeInfo) -> QWidget:
        ...
        if anime.poster_path:
            try:
                ...
                if response.status_code == 200:
                    ...
                    poster_label.setPixmap(pixmap.scaled(60, 90, Qt.KeepAspectRatio, Qt.SmoothTransformation))
            except:
        ...
        # 개요
        if anime.overview:
            overview_label = QLabel(anime.overview[:100] + "..." if len(anime.overview) > 100 else anime.overview)
