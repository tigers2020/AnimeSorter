        ...
        try:
            ...
            for i, file_path in enumerate(self.file_paths):
                ...
                try:
                    ...
                    self.error_occurred.emit(f"파일 처리 오류: {os.path.basename(file_path)} - {str(e)}")
...
class FileProcessingController(IController):
    ...
    def __init__(self, event_bus: IEventBus, parent_widget: QObject = None):
        ...
        # 워커 스레드
        self.processing_worker: Optional[FileProcessingWorker] = None
    ...
    def set_source_files(self, file_paths: List[str]) -> None:
        ...
        self.event_bus.publish("status_update", {
            'message': f"{len(valid_files)}개 파일 선택됨"
        })
    ...
    def start_processing(self) -> None:
        ...
        try:
            ...
            if not files_to_process:
                self.event_bus.publish("error_occurred", "처리할 파일이 없습니다. 먼저 파일이나 폴더를 선택해주세요.")
                return
    ...
    def _on_processing_completed(self, parsed_items: List[ParsedItem]) -> None:
        ...
        # 결과 이벤트 발행
        self.event_bus.publish("processing_completed", {
