...
from ..components.organize_progress_dialog import OrganizeProgressDialog, OrganizeResult
...
class OrganizeController(IController):
    ...
    def __init__(self, event_bus: IEventBus, parent_widget: QObject = None):
        ...
        # 대상 디렉토리
        self.destination_directory: Optional[str] = None
        
        # 그룹화된 아이템들
        self.grouped_items: Dict[str, List] = {}
        
        # 정리 결과
        self.last_organize_result: Optional[OrganizeResult] = None
    ...
    def _on_organize_completed(self, result: OrganizeResult, execute_mode: bool = True) -> None:
        ...
        try:
            ...
            # 완료 이벤트 발행
            self.event_bus.publish("organize_completed", {
                'result': result,
            ...
            self.logger.info(f"{operation_name} 완료: 성공 {result.success_count}, 실패 {result.error_count}")
    ...
    def _on_organize_failed(self, error_message: str) -> None:
        ...
        # 실패 이벤트 발행
        self.event_bus.publish("organize_failed", {
        ...
        # 상태 업데이트
        self.event_bus.publish("status_update", {
            'message': f"{operation_name} 실패: {error_message}"
        })
    ...
    def _on_organize_cancelled(self) -> None:
        ...
        # 취소 이벤트 발행
        self.event_bus.publish("organize_cancelled", {
            'operation': self.current_operation
        })
        
        # 상태 업데이트
        self.event_bus.publish("status_update", {
            'message': f"{operation_name}이 취소되었습니다"
        })
    ...
    def get_organize_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_result': self.last_organize_result.__dict__ if self.last_organize_result else None
        }
