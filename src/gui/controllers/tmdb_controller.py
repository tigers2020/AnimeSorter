...
class TMDBController(IController):
    ...
    def __init__(self, event_bus: IEventBus, parent_widget: QObject = None):
        ...
        # 순차적 검색 관리
        self.pending_search_groups: List[Tuple[str, str]] = []  # (group_id, title)
        self.is_batch_searching = False
        
        # 매칭 결과 저장
        self.tmdb_matches: Dict[str, TMDBAnimeInfo] = {}  # group_id -> TMDBAnimeInfo
        ...
        self.config = {
            ...
            'search_delay_ms': 500,  # 연속 검색 간 지연
    ...
    def _show_search_dialog(self, group_id: str, title: str, search_results: List[TMDBAnimeInfo]) -> None:
        """검색 다이얼로그 표시"""
        try:
            ...
            if len(self.search_dialogs) >= self.config['max_concurrent_dialogs']:
                self.logger.warning(f"최대 다이얼로그 수 제한 ({self.config['max_concurrent_dialogs']})")
                return
            ...
            # 시그널 연결
            dialog.anime_selected.connect(lambda anime: self._handle_anime_selection({
                'group_id': group_id,
                'anime': anime
            }))
            
            dialog.search_cancelled.connect(lambda: self._handle_search_cancellation({
                'group_id': group_id
            }))
    ...
    def _handle_anime_selection(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 선택 완료 이벤트 발행
            self.event_bus.publish("tmdb_match_completed", {
                'group_id': group_id,
                'anime': anime
            })
    ...
    def _process_next_search(self) -> None:
        ...
        try:
            if not self.is_batch_searching or not self.pending_search_groups:
                ...
                if self.is_batch_searching:
                    self.is_batch_searching = False
                    self.event_bus.publish("tmdb_batch_search_completed", {
                        'matches_count': len(self.tmdb_matches)
                    })
            ...
            # 지연 후 검색 시작
            QTimer.singleShot(self.config['search_delay_ms'], lambda: self._handle_search_request({
                'group_id': group_id,
                'title': group_title
            }))
    ...
    def get_match_for_group(self, group_id: str) -> Optional[TMDBAnimeInfo]:
        """그룹의 TMDB 매칭 결과 반환"""
    ...
    def set_match_for_group(self, group_id: str, anime: TMDBAnimeInfo) -> None:
        """그룹의 TMDB 매칭 결과 설정"""
        self.tmdb_matches[group_id] = anime
        self.event_bus.publish("tmdb_match_updated", {
            'group_id': group_id,
            'anime': anime
        })
    ...
    def clear_match_for_group(self, group_id: str) -> None:
        ...
        if group_id in self.tmdb_matches:
            ...
            self.event_bus.publish("tmdb_match_cleared", {
                'group_id': group_id
            })
