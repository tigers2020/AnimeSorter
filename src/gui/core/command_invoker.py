...
from ..interfaces.i_command import ICommand, ICommandInvoker, CommandState
...
class CommandInvoker(ICommandInvoker):
    ...
    def get_current_command(self) -> Optional[ICommand]:
        """현재 명령 가져오기"""
        if self._current_position < 0:
            return None
        return self._command_history[self._current_position]
    
    def get_next_command(self) -> Optional[ICommand]:
        """다음 명령 가져오기"""
        if self._current_position + 1 >= len(self._command_history):
            return None
    ...
    def get_stats(self) -> dict:
        """통계 정보 반환"""
        completed_count = sum(1 for cmd in self._command_history if cmd.state == CommandState.COMPLETED)
