...
from src.app import (BackupCompletedEvent, BackupFailedEvent,
                     ...
                     RedoExecutedEvent, SafetyAlertEvent,
                     SafetyStatusUpdateEvent, ScanStatus, TaskCancelledEvent,
                     TaskCompletedEvent, TaskFailedEvent, TaskProgressEvent,
                     TaskStartedEvent, TMDBMatchFoundEvent,
                     TMDBSearchCompletedEvent, TMDBSearchStartedEvent,
                     TransactionCommittedEvent, TransactionStartedEvent,
...
class EventHandlerManager:
    ...
    def setup_event_subscriptions(self):
        ...
        try:
            ...
            # Journal System 이벤트 구독
            event_bus.subscribe(
                JournalEntryCreatedEvent, self.on_journal_entry_created, weak_ref=False
            )
            event_bus.subscribe(
                TransactionStartedEvent, self.on_transaction_started, weak_ref=False
            )
            event_bus.subscribe(
                TransactionCommittedEvent, self.on_transaction_committed, weak_ref=False
            )
            self.logger.info("✅ Journal System 이벤트 구독 설정")

            # Undo/Redo System 이벤트 구독
    ...
    # Journal System 이벤트 핸들러들
    def on_journal_entry_created(self, event: JournalEntryCreatedEvent):
        """저널 항목 생성 이벤트 핸들러"""
        self.logger.info(f"📝 [MainWindow] 저널 항목 생성: {event.entry_id}")

    def on_transaction_started(self, event: TransactionStartedEvent):
        """트랜잭션 시작 이벤트 핸들러"""
        self.logger.info(f"🔄 [MainWindow] 트랜잭션 시작: {event.transaction_id}")

    def on_transaction_committed(self, event: TransactionCommittedEvent):
        """트랜잭션 커밋 이벤트 핸들러"""
        self.logger.info(f"✅ [MainWindow] 트랜잭션 커밋: {event.transaction_id}")

    # Undo/Redo System 이벤트 핸들러들
