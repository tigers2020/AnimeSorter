...
from PyQt5.QtWidgets import (
    ...
    QApplication, QMenuBar, QMenu, QAction, QStatusBar, QLabel, QProgressBar,
    QDialog
)
...
# UI Components
from .components import MainToolbar, LeftPanel, RightPanel, ResultsView, SettingsDialog
...
# Table Models
from .table_models import GroupedListModel, DetailFileModel
...
class MainWindow(QMainWindow):
    ...
    def create_main_content(self, parent_layout):
        ...
        # 모델들 초기화
        # 대상 폴더 정보 가져오기
        self.destination_directory = self.settings_manager.get_setting('destination_root', '대상 폴더')
        
        self.grouped_model = GroupedListModel({}, self.tmdb_client, self.destination_directory)  # 그룹 리스트용
    ...
    def on_tmdb_search_requested(self, group_id: str):
        ...
        try:
            ...
            dialog.anime_selected.connect(lambda anime: self.on_tmdb_anime_selected(group_id, anime))
    ...
    def restore_table_column_widths(self):
        ...
        try:
            ...
            if column_widths and hasattr(self, 'right_panel') and hasattr(self.right_panel, 'results_view'):
    ...
    def process_selected_files(self, file_paths: List[str]):
            ...
            try:
                ...
                try:
                    ...
                    if file_size < 1024 * 1024:  # 1MB 미만
                        print(f"⚠️ 비디오 파일 크기가 너무 작음 (제외): {os.path.basename(file_path)} ({file_size} bytes)")
                        self.right_panel.add_activity_log(f"⚠️ 제외됨: {os.path.basename(file_path)} (크기: {file_size} bytes)")
                        continue
                except OSError:
                    print(f"⚠️ 파일 크기 확인 실패 (제외): {os.path.basename(file_path)}")
                    self.right_panel.add_activity_log(f"⚠️ 제외됨: {os.path.basename(file_path)} (파일 접근 불가)")
                    continue
        ...
        if parsed_items:
            ...
            self.left_panel.update_stats(stats['total'], stats['parsed'], stats['pending'], stats['groups'])
                ...
                for file in files:
                    if file.lower().endswith(video_extensions):
                        ...
                        try:
                            ...
                            if file_size < 1024 * 1024:  # 1MB 미만
                                print(f"⚠️ 비디오 파일 크기가 너무 작음 (제외): {file} ({file_size} bytes)")
                                self.right_panel.add_activity_log(f"⚠️ 제외됨: {file} (크기: {file_size} bytes)")
                                continue
                        except OSError:
    ...
    def start_tmdb_search_for_groups(self):
        ...
        try:
            ...
            if self.pending_tmdb_groups:
                print(f"🔍 {len(self.pending_tmdb_groups)}개 그룹에 대해 순차적 TMDB 검색을 시작합니다")
                self.update_status_bar(f"TMDB 검색 시작: {len(self.pending_tmdb_groups)}개 그룹 (순차적 처리)")
                # 첫 번째 그룹부터 시작
    ...
    def process_next_tmdb_group(self):
        ...
        print(f"🔍 TMDB 검색 시작: '{group_title}' (그룹 {group_id}) - {len(self.pending_tmdb_groups)}개 남음")
        self.update_status_bar(f"TMDB 검색 중: {group_title} ({len(self.pending_tmdb_groups)}개 남음)")
    ...
    def start_file_organization(self):
        ...
        try:
            ...
            if not hasattr(self, 'anime_data_manager'):
                QMessageBox.warning(self, "경고", "스캔된 데이터가 없습니다. 먼저 파일을 스캔해주세요.")
                return
                
            grouped_items = self.anime_data_manager.get_grouped_items()
            if not grouped_items:
                QMessageBox.warning(self, "경고", "정리할 그룹이 없습니다. 먼저 파일을 스캔해주세요.")
                return
                
            # 대상 폴더 확인
            if not self.destination_directory or not os.path.exists(self.destination_directory):
                QMessageBox.warning(self, "경고", "대상 폴더가 설정되지 않았거나 존재하지 않습니다.")
                return
    ...
    def on_organize_proceed(self):
        ...
        try:
            ...
            # 진행률 다이얼로그 생성 및 실행
            progress_dialog = OrganizeProgressDialog(grouped_items, self.destination_directory, self)
    ...
    def on_organization_completed(self, result):
        ...
        try:
            ...
            print(f"✅ 파일 정리 완료: 성공 {result.success_count}, 실패 {result.error_count}, 건너뜀 {result.skip_count}")
