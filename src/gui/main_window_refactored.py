...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.file_processing_controller: Optional[FileProcessingController] = None
        self.tmdb_controller: Optional[TMDBController] = None
        self.organize_controller: Optional[OrganizeController] = None
        
        # 뷰모델들
        self.main_window_view_model: Optional[MainWindowViewModel] = None
        self.file_list_view_model: Optional[FileListViewModel] = None
        self.detail_view_model: Optional[DetailViewModel] = None
        
        # UI 컴포넌트들
        self.toolbar: Optional[MainToolbar] = None
    ...
    def _initialize_architecture(self):
        ...
        self.file_processing_controller = self.component_factory.create_controller("file_processing_controller")
        ...
        # 5. 뷰모델들 생성
        self.main_window_view_model = self.component_factory.create_view_model("main_window_view_model")
    ...
    def _connect_signals(self):
        ...
        if self.main_window_view_model:
            ...
            self.main_window_view_model.status_message_changed.connect(self._on_status_message_changed)
            self.main_window_view_model.error_message_changed.connect(self._on_error_message_changed)
        
        if self.file_list_view_model:
            self.file_list_view_model.data_changed.connect(self._on_file_list_data_changed)
            self.file_list_view_model.selection_changed.connect(self._on_file_list_selection_changed)
    ...
    def _on_command_failed(self, command_info):
        """명령 실행 실패 처리"""
        self.logger.error(f"명령 실행 실패: {command_info.get('command_name')} - {command_info.get('error')}")
