...
from .anime_data_manager import ParsedItem, AnimeDataManager
...
class FileProcessingManager:
        ...
        for i, file_path in enumerate(file_paths):
            try:
                ...
                if parsed_metadata and parsed_metadata.title:
                    ...
                    print(f"✅ 파싱 성공: {parsed_metadata.title} S{parsed_item.season:02d}E{parsed_item.episode:02d}")
                else:
                    # 파싱 실패
                    parsed_item = ParsedItem(
                        sourcePath=file_path,
    ...
    def create_processing_plans(self, parsed_items: List[ParsedItem], 
                               naming_scheme: str = 'standard') -> List[FileProcessingPlan]:
        """파일 처리 계획 생성"""
        if not parsed_items:
            return []
        ...
        for item in parsed_items:
            ...
            try:
                ...
                # 파일 크기 계산
                file_size = os.path.getsize(item.sourcePath) if os.path.exists(item.sourcePath) else 0
                size_mb = file_size // (1024 * 1024)
                total_size += size_mb
    ...
    def simulate_processing(self) -> Dict[str, any]:
        ...
        print(f"✅ 시뮬레이션 완료: {simulation_results['success_count']}개 성공, {simulation_results['error_count']}개 충돌")
        return simulation_results
    
    def execute_processing(self, dry_run: bool = True, progress_callback: Optional[Callable[[int], None]] = None, max_workers: int = 4) -> Dict[str, any]:
        ...
        def process_one(plan: FileProcessingPlan) -> Tuple[bool, FileProcessingPlan, Optional[str]]:
            try:
                if plan.conflicts:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_plan = {executor.submit(process_one, plan): plan for plan in self.processing_plans}
        ...
        print(f"✅ 파일 처리 완료: {results['total_processed']}개 성공, {results['total_errors']}개 오류")
        return results
    ...
    def _execute_single_plan(self, plan: FileProcessingPlan) -> bool:
        ...
        try:
            ...
            print(f"✅ 파일 처리 완료: {os.path.basename(plan.source_path)} → {os.path.basename(plan.target_path)}")
            return True
    ...
    def create_processing_plan(self, parsed_items: List[ParsedItem], organize_mode: str = "move") -> List[FileProcessingPlan]:
        ...
        for item in parsed_items:
            try:
                ...
                plan = FileProcessingPlan(
                    ...
                    estimated_size=os.path.getsize(item.sourcePath or item.path) if os.path.exists(item.sourcePath or item.path) else None
                )
    ...
    def simulate_organization(self, parsed_items: List[ParsedItem], organize_mode: str = "move") -> Dict:
        """파일 정리 시뮬레이션 실행"""
        print(f"🎭 파일 처리 시뮬레이션 시작")
        ...
        print(f"✅ 시뮬레이션 완료: {simulation_results['successful']}개 성공, {simulation_results['conflicts']}개 충돌")
        return simulation_results
