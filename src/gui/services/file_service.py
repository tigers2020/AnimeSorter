...
class FileService(IService):
    ...
    def _handle_file_operation(self, event) -> None:
        ...
        try:
            ...
            if operation_type == 'move':
                ...
                self.event_bus.publish("file_operation_completed", {
                    'type': 'move',
                    'result': result
                })
    ...
    def _handle_directory_scan(self, event) -> None:
        ...
        try:
            ...
            self.event_bus.publish("directory_scan_completed", {
                'directory': directory,
                'files': files
            })
    ...
    def _handle_file_validation(self, event) -> None:
        ...
        try:
            ...
            self.event_bus.publish("file_validation_completed", {
        ...
        try:
            ...
            if file_info.is_video:
                ...
                if file_info.size < min_size:
                    validation_result['issues'].append(f"파일 크기가 너무 작습니다 ({file_info.size} bytes)")
            
            # 경로 길이 확인
            max_path_length = self.get_config('max_path_length', 260)
            if len(file_info.path) > max_path_length:
                validation_result['issues'].append(f"경로가 너무 깁니다 ({len(file_info.path)} 문자)")
        ...
        try:
            ...
            for root, dirs, filenames in os.walk(directory):
                for filename in filenames:
                    ...
                    # 파일 타입 필터링
                    include_file = False
                    if 'video' in file_types and file_info.is_video:
                        include_file = True
                    elif 'subtitle' in file_types and file_info.is_subtitle:
                        include_file = True
                    elif 'all' in file_types:
                        include_file = True
    ...
    def move_file(self, source_path: str, destination_path: str, 
                  move_subtitles: bool = True) -> MoveOperation:
        """파일 이동"""
        try:
