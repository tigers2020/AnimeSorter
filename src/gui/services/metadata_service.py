...
from src.core.tmdb_client import TMDBClient, TMDBAnimeInfo
...
class MetadataSearchResult:
    """메타데이터 검색 결과"""
    success: bool
    anime_info: Optional[TMDBAnimeInfo] = None
...
class MetadataService(QObject, IService):
    ...
    async def search_anime_async(self, title: str, year: Optional[int] = None) -> MetadataSearchResult:
        """비동기 애니메이션 검색"""
        try:
            # 캐시 확인
    ...
    def search_anime_sync(self, title: str, year: Optional[int] = None) -> MetadataSearchResult:
        """동기 애니메이션 검색 (비동기 래퍼)"""
        try:
    ...
    def get_anime_details(self, tmdb_id: int) -> Optional[TMDBAnimeInfo]:
