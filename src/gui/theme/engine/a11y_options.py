...
class A11yOptions(QObject):
    ...
    def __init__(self, app: QApplication = None, config_path: Optional[Union[str, Path]] = None):
        ...
        self.config_path = Path(config_path) if config_path else Path(__file__).parent.parent / "config" / "a11y.json"
    ...
    def _setup_defaults(self) -> None:
        ...
        self.settings.high_contrast_enabled = self.qsettings.value("a11y/high_contrast_enabled", False, type=bool)
        self.settings.font_size_level = FontSizeLevel(self.qsettings.value("a11y/font_size_level", "normal"))
        self.settings.color_contrast_enhancement = self.qsettings.value("a11y/color_contrast_enhancement", False, type=bool)
    ...
    def _update_font_size_multiplier(self) -> None:
        ...
        self.settings.font_size_multiplier = size_multipliers.get(self.settings.font_size_level, 1.0)
    ...
    def validate_color_contrast(self, foreground: Union[str, QColor], 
                               background: Union[str, QColor]) -> Dict[str, Any]:
        """색상 대비 검증"""
        try:
            # QColor를 문자열로 변환
    ...
    def enhance_color_contrast(self, foreground: Union[str, QColor], 
                              background: Union[str, QColor]) -> Optional[Union[str, QColor]]:
        """색상 대비 강화"""
    ...
    def get_accessible_color_palette(self, base_color: Union[str, QColor], 
                                   theme: str = "light") -> Dict[str, str]:
        """접근성을 고려한 색상 팔레트 생성"""
        try:
        ...
        try:
            ...
            for accent_name, accent_color in accent_colors[:2]:
                for bg_name, bg_color in background_colors[:2]:
                    test_combinations.append((f"bg:{bg_name}", bg_color, f"accent:{accent_name}", accent_color))
            ...
            for bg_desc, bg_color, fg_desc, fg_color in test_combinations:
                ...
                if not contrast_result.get("passes_aa", False):
                    contrast_issues.append({
                        ...
                        "contrast_ratio": contrast_result.get("contrast_ratio", 0),
                        "required": 4.5,
                        "suggestions": self.color_utils.get_contrast_enhancement_suggestions(fg_color, bg_color)
                    })
            ...
            validation_result["accessibility_score"] = (passed_checks / total_checks * 100) if total_checks > 0 else 0
            
            # WCAG 준수 여부
            validation_result["wcag_compliance"]["aa"] = len(contrast_issues) == 0
            validation_result["wcag_compliance"]["aaa"] = all(
                contrast_analysis[combo]["passes_aaa"] 
                for combo in contrast_analysis.keys()
            )
            
            # 전체 등급 결정
            validation_result["detailed_analysis"]["overall_rating"] = self._determine_overall_rating(
                validation_result["accessibility_score"],
                validation_result["wcag_compliance"]
            )
    ...
    def _validate_font_accessibility(self, font_tokens: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                ...
                try:
                    ...
                    if isinstance(size_value, str) and "px" in size_value:
                        ...
                        if size_num < min_size:
                            analysis["issues"].append(f"폰트 크기가 너무 작습니다: {size_name} = {size_value}")
                            analysis["valid"] = False
                except (ValueError, TypeError):
                    analysis["issues"].append(f"폰트 크기 형식이 잘못되었습니다: {size_name} = {size_value}")
                    analysis["valid"] = False
            ...
            if analysis["issues"]:
                ...
                analysis["recommendations"].append("폰트 패밀리는 fallback 폰트를 포함하여 정의하세요.")
                ...
                try:
                    if isinstance(spacing_value, str) and "px" in spacing_value:
                        ...
                        if spacing_num < min_spacing:
                            analysis["issues"].append(f"간격이 너무 작습니다: {spacing_name} = {spacing_value}")
                            analysis["valid"] = False
                except (ValueError, TypeError):
                    analysis["issues"].append(f"간격 형식이 잘못되었습니다: {spacing_name} = {spacing_value}")
                    analysis["valid"] = False
            
            # 권장사항 생성
            if analysis["issues"]:
                analysis["recommendations"].append("요소 간 간격은 최소 4px 이상으로 설정하세요.")
                analysis["recommendations"].append("터치 인터페이스를 위한 충분한 간격을 확보하세요.")
    ...
    def _determine_overall_rating(self, accessibility_score: float, 
                                 wcag_compliance: Dict[str, bool]) -> str:
        """전체 접근성 등급 결정"""
    ...
    def _generate_validation_recommendations(self, validation_result: Dict[str, Any],
                                           contrast_issues: List[Dict[str, Any]],
                                           font_analysis: Dict[str, Any],
                                           spacing_analysis: Dict[str, Any]) -> List[str]:
        """검증 결과를 바탕으로 권장사항 생성"""
        recommendations = []
        
        # 색상 대비 관련 권장사항
        if contrast_issues:
            recommendations.append(f"{len(contrast_issues)}개의 색상 조합이 WCAG AA 기준을 충족하지 않습니다.")
        ...
        if accessibility_score < 80:
            ...
            recommendations.append("접근성을 더욱 향상시킬 수 있습니다. 세부적인 조정을 고려하세요.")
        
        # WCAG 준수 관련 권장사항
        if not validation_result["wcag_compliance"].get("aa", False):
            recommendations.append("WCAG AA 기준을 충족하지 않습니다. 기본적인 접근성 요구사항을 만족해야 합니다.")
        
        if not validation_result["wcag_compliance"].get("aaa", False):
            recommendations.append("WCAG AAA 기준을 충족하지 않습니다. 고대비 모드 사용자를 위한 추가 개선이 필요합니다.")
...
# 편의 함수들
def create_a11y_options(app: QApplication = None, config_path: Optional[Union[str, Path]] = None) -> A11yOptions:
    """A11yOptions 인스턴스 생성 편의 함수"""
    return A11yOptions(app, config_path)


def validate_theme_accessibility(theme_file_path: Union[str, Path], 
                                a11y_options: Optional[A11yOptions] = None) -> Dict[str, Any]:
    """테마 파일 접근성 검증 편의 함수"""
