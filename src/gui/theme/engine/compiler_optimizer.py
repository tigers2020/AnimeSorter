...
class CompilerOptimizer:
    ...
    def optimize_ast(self, ast_node: ASTNode, level: OptimizationLevel = None) -> Tuple[ASTNode, OptimizationResult]:
        """
        AST를 최적화합니다
        ...
        try:
            ...
            result = OptimizationResult(
                ...
                performance_improvement=self._calculate_performance_improvement(ast_node, optimized_ast)
            )
            ...
            logger.info(f"AST 최적화 완료: {result.removed_nodes}개 노드 제거, "
    ...
    def _run_optimization_pipeline(self, ast_node: ASTNode, level: OptimizationLevel) -> ASTNode:
        ...
        try:
            ...
            for iteration in range(self.settings['max_iterations']):
                ...
                logger.debug(f"최적화 반복 {iteration + 1} 완료: "
                    ...
                    if len(properties) > 1:
                        ...
                        for other_property in properties[1:]:
                            ...
                            if ',' in main_property.value:
                                ...
                                main_property.value = f"{main_property.value}, {other_property.value}"
        ...
        try:
            ...
            for selector, rules in selector_groups.items():
                if len(rules) > 1:
                    ...
                    # 통계 업데이트
                    self.optimization_stats['merged_rules'] = self.optimization_stats.get('merged_rules', 0) + len(rules) - 1
        ...
        try:
            ...
            for existing_property in rule_node.get_children_by_type(NodeType.PROPERTY):
                if existing_property.value == property_node.value:
                    if existing_property.children and existing_property.children[0].value == property_value:
                        return True
        ...
        try:
            ...
            if len(properties) > 1:
                ...
                sorted_properties = sorted(
                    properties,
                    key=lambda p: property_priority.get(p.value.lower(), 999)
                )
                
                # 정렬된 순서로 재배치
                rule_node.children = [child for child in rule_node.children if child.node_type != NodeType.PROPERTY]
    ...
    def _calculate_performance_improvement(self, original_ast: ASTNode, optimized_ast: ASTNode) -> float:
        """성능 향상률을 계산합니다"""
        try:
    ...
    def export_optimization_report(self, output_path: Optional[Path] = None) -> str:
        ...
        try:
            ...
            report = {
                ...
                'enabled_rules': [name for name, rule in self.optimization_rules.items() if rule.enabled],
...
def optimize_ast(ast_node: ASTNode, compiler: TemplateCompiler, 
                level: OptimizationLevel = OptimizationLevel.ADVANCED) -> Tuple[ASTNode, OptimizationResult]:
    """AST를 최적화합니다"""
