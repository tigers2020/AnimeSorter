...
class DynamicQSSEngine:
    ...
    def generate_dynamic_qss(
        ...
        try:
            ...
            logger.info(
                f"동적 QSS 생성 완료: {len(final_qss)}자, " f"소요시간: {generation_time:.4f}초"
            )
    ...
    def _apply_dynamic_styles(self, context: dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(
                f"동적 스타일 적용 시작: {len(self.dynamic_styles)}개 스타일, 컨텍스트: {context}"
            )
            ...
            for style in self.dynamic_styles:
                ...
                if should_apply:
                    ...
                    if qss:
                        ...
                        logger.debug("  - QSS 생성 실패")
                else:
                    logger.debug("  - 조건 불만족으로 스킵")
    ...
    def _generate_device_specific_styles(self, device_type: str, context: dict[str, Any]) -> str:
        ...
        try:
            device_styles = {
                "mobile": {"font-size": "14px", "padding": "8px", "margin": "4px"},
                "tablet": {"font-size": "16px", "padding": "12px", "margin": "8px"},
                "desktop": {"font-size": "18px", "padding": "16px", "margin": "12px"},
                "large_desktop": {"font-size": "20px", "padding": "20px", "margin": "16px"},
            }
    ...
    def _optimize_qss(self, qss: str, context: dict[str, Any]) -> str:
        ...
        try:
            ...
            if base_qss.strip():
                ...
                logger.debug(
                    f"QSS 최적화 완료: {result.removed_nodes}개 노드 제거, "
    ...
    def create_media_query_style(
        self, selector: str, properties: dict[str, str], media_query: MediaQuery, priority: int = 0
    ) -> DynamicStyle:
        """미디어 쿼리 스타일을 생성합니다"""
        try:
            style = DynamicStyle(
        ...
        try:
            ...
            for style in self.dynamic_styles:
                style_data = {
                    ...
                    "media_queries": [
                        {"type": query.query_type.value, "custom_query": query.custom_query}
...
# 편의 함수들
def create_dynamic_qss_engine(
    compiler: TemplateCompiler, optimizer: Optional[CompilerOptimizer] = None
) -> DynamicQSSEngine:
...
def create_style_condition(
    condition_type: ConditionType, property_name: str, value: Any, operator: str = "=="
) -> StyleCondition:
    """StyleCondition 인스턴스를 생성합니다"""
    return StyleCondition(condition_type, property_name, value, operator)


def create_media_query(
    query_type: MediaQueryType, custom_query: Optional[str] = None
) -> MediaQuery:
