...
class IconManager(QObject):
    ...
    def __init__(self, app: QApplication, base_path: Optional[Union[str, Path]] = None):
        ...
        self.app = app
        self.base_path = (
            Path(base_path) if base_path else Path(__file__).parent.parent / "assets" / "icons"
        )
    ...
    def _create_icon_from_file(
        self, file_path: Path, size: Optional[QSize] = None
    ) -> Optional[QIcon]:
        """파일에서 아이콘 생성"""
        try:
    ...
    def add_icon(
        self, icon_name: str, icon_path: Union[str, Path], theme: Optional[str] = None
    ) -> bool:
        """
        새 아이콘 추가
