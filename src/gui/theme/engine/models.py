...
class BorderToken(BaseModel):
    ...
    def validate_border_style(cls, v):
        """테두리 스타일 검사"""
        if v is not None:
            valid_styles = ['none', 'hidden', 'dotted', 'dashed', 'solid', 'double', 'groove', 'ridge', 'inset', 'outset']
            if v not in valid_styles:
...
def create_shadow_token(offset_x: Union[int, float], offset_y: Union[int, float], 
                       color: str, **kwargs) -> ShadowToken:
    """그림자 토큰 생성 헬퍼"""
