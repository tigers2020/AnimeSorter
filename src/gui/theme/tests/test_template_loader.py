...
from src.gui.theme.engine.types import (
    ThemeTokens, ColorToken, FontToken, SpacingToken, BorderToken,
    ColorPalette, FontSystem, SpacingSystem, BorderSystem
)
...
class TestTemplateLoader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_tokens = ThemeTokens(
            ...
            fonts=FontSystem(
                primary=FontToken(family="Segoe UI, sans-serif", size=14, description="Primary font"),
                sizes={
                    "xs": 12,
                    "sm": 14,
                    "md": 16,
    ...
    def _create_test_templates(self):
        ...
        """
        (self.templates_dir / "components" / "button.qss").write_text(button_content, encoding='utf-8')
        ...
        """
        (self.templates_dir / "layouts" / "main_window.qss").write_text(main_window_content, encoding='utf-8')
        ...
        """
        (self.templates_dir / "utilities" / "utilities.qss").write_text(utilities_content, encoding='utf-8')
    ...
    def test_default_functions_registration(self):
        ...
        # 기본 함수들이 등록되었는지 확인
        expected_functions = [
            'theme', 'color', 'spacing', 'font', 'border',
            'shadow', 'transition', 'z_index', 'breakpoint',
            'math', 'scale', 'mix', 'lighten', 'darken',
            'alpha', 'contrast'
        ]
